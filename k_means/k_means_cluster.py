...
k = 3  # Puoi modificare questo numero per esplorare diversi cluster
...
def get_max_min_by_values(value):
    ...
    for person, features in data_dict.items():
        value_stock = features.get(value, "NaN")
        
        # Controlla che il valore non sia 'NaN'
        if value_stock != 'NaN':
            if value_stock > max_value:
                max_value = value_stock
            if value_stock < min_value:
                min_value = value_stock

    print(f"Max {value}:", max_value)
    print(f"Min {value}:", min_value)
    
get_max_min_by_values("exercised_stock_options")
get_max_min_by_values("salary")